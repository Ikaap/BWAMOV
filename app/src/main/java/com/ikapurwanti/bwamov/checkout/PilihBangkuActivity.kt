...
class PilihBangkuActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        a3.setOnClickListener{
            if (statusA3) {
                a3.setImageResource(R.drawable.rectangle_line)
                statusA3 = false
                total -= 1
                beliTiket(total)
            } else {
                a3.setImageResource(R.drawable.rectangle_pink)
                statusA3 = true
                total += 1
                beliTiket(total)
        ...
        a4.setOnClickListener{
            if (statusA4) {
                a4.setImageResource(R.drawable.rectangle_line)
                statusA4 = false
                total -= 1
                beliTiket(total)
            } else {
                a4.setImageResource(R.drawable.rectangle_pink)
                statusA4 = true
                total += 1
                beliTiket(total)
    ...
    // fungsi dari function ini untuk melakukan pengecekan , buttonnya nanti akan di disable(hilang), kl beli btnnya akan muncul
